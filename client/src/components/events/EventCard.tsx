...
export default function EventCard({ event }: EventCardProps) {
  const isEventPast = event.date ? new Date(event.date) < new Date() : false;
  const spotsLeft = getAvailableSpots(event.capacity, event.registration_count);
  const isSoldOut = spotsLeft === 0;

  return (
    <div className="group bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-purple-200 transition-all duration-300 transform hover:-translate-y-2">
      {/* Event Image */}
