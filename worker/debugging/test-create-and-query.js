...
async function testCreateAndQuery() {
  console.log('🔍 Testing Create Event and Query');
  ...
  try {
    ...
    const createResponse = await fetch(`${STAGING_API}/api/auth/register`, {
      ...
      body: JSON.stringify({
        email: `create-query-${Date.now()}@example.com`,
        name: `Create Query User ${Date.now()}`,
    ...
    const upgradeResponse = await fetch(`${STAGING_API}/api/auth/upgrade-to-organizer`, {
      ...
      body: JSON.stringify({
        organization_name: 'Create Query Org',
    ...
    const createEventResponse = await fetch(`${STAGING_API}/api/events`, {
      ...
      body: JSON.stringify({
        name: 'Create Query Test Event',
        details: 'Testing create and query flow',
